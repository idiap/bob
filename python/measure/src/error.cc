...
void bind_measure_error() {
 ...
 def("correctlyClassifiedPositives", &correctlyClassifiedPositives, (arg("positives"), arg("threshold")), "This method returns a blitz::Array composed of booleans that pin-point which positives where correctly classified in a 'positive' score sample, given a threshold. It runs the formula: foreach (element k in positive) if positive[k] >= threshold: returnValue[k] = true else: returnValue[k] = false");
 def("correctlyClassifiedNegatives", &correctlyClassifiedNegatives, (arg("negatives"), arg("threshold")), "This method returns a blitz::Array composed of booleans that pin-point which negatives where correctly classified in a 'negative' score sample, given a threshold. It runs the formula: foreach (element k in negative) if negative[k] < threshold: returnValue[k] = true else: returnValue[k] = false");
 def("eerThreshold", &eerThreshold, (arg("negatives"), arg("positives")), "Calculates the threshold that is, as close as possible, to the equal-error-rate (EER) given the input data. The EER should be the point where the FAR equals the FRR. Graphically, this would be equivalent to the intersection between the R.O.C. (or D.E.T.) curves and the identity.");
 def("minWeightedErrorRateThreshold", &minWeightedErrorRateThreshold, (arg("negatives"), arg("positives"), arg("cost")), "Calculates the threshold that minimizes the error rate, given the input data. An optional parameter 'cost' determines the relative importance between false-accepts and false-rejections. This number should be between 0 and 1 and will be clipped to those extremes. The value to minimize becomes: ER_cost = [cost * FAR] + [(1-cost) * FRR]. The higher the cost, the higher the importance given to *not* making mistakes classifying negatives/noise/impostors.");
