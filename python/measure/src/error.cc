...
static blitz::Array<bool,1> 
  correctlyClassifiedPositives(tp::const_ndarray positives, double threshold) {
  return err::correctlyClassifiedPositives(positives.bz<double,1>(), threshold);
}

static blitz::Array<bool,1> 
  correctlyClassifiedNegatives(tp::const_ndarray negatives, double threshold) {
  return err::correctlyClassifiedNegatives(negatives.bz<double,1>(), threshold);
}

static double eerThreshold(tp::const_ndarray negatives,
    tp::const_ndarray positives) {
  return err::eerThreshold(negatives.bz<double,1>(), positives.bz<double,1>());
}

static double minWeightedErrorRateThreshold(tp::const_ndarray negatives, 
    tp::const_ndarray positives, double cost) {
  return err::minWeightedErrorRateThreshold(negatives.bz<double,1>(), positives.bz<double,1>(), cost);
}

static double minHterThreshold(tp::const_ndarray negatives,
    tp::const_ndarray positives) {
  return err::minHterThreshold(negatives.bz<double,1>(), 
      positives.bz<double,1>());
}

static blitz::Array<double,2> roc (tp::const_ndarray negatives,
    tp::const_ndarray positives, size_t points) {
  return err::roc(negatives.bz<double,1>(), positives.bz<double,1>(), points);
}

static blitz::Array<double,2> det (tp::const_ndarray negatives,
    tp::const_ndarray positives, size_t points) {
  return err::det(negatives.bz<double,1>(), positives.bz<double,1>(), points);
}

static blitz::Array<double,2> epc (tp::const_ndarray dev_negatives,
    tp::const_ndarray dev_positives, tp::const_ndarray test_negatives, 
    tp::const_ndarray test_positives, size_t points) {
  return err::epc(dev_negatives.bz<double,1>(), dev_positives.bz<double,1>(), 
      test_negatives.bz<double,1>(), test_positives.bz<double,1>(), points);
}
...
void bind_measure_error() {
 ...
 def("correctly_classified_positives", &correctlyClassifiedPositives, (arg("positives"), arg("threshold")), "This method returns a blitz::Array composed of booleans that pin-point which positives where correctly classified in a 'positive' score sample, given a threshold. It runs the formula: foreach (element k in positive) if positive[k] >= threshold: returnValue[k] = true else: returnValue[k] = false");
 def("correctly_classified_negatives", &correctlyClassifiedNegatives, (arg("negatives"), arg("threshold")), "This method returns a blitz::Array composed of booleans that pin-point which negatives where correctly classified in a 'negative' score sample, given a threshold. It runs the formula: foreach (element k in negative) if negative[k] < threshold: returnValue[k] = true else: returnValue[k] = false");
 def("eer_threshold", &eerThreshold, (arg("negatives"), arg("positives")), "Calculates the threshold that is as close as possible to the equal-error-rate (EER) given the input data. The EER should be the point where the FAR equals the FRR. Graphically, this would be equivalent to the intersection between the ROC (or DET) curves and the identity.");
 def("min_weighted_error_rate_threshold", &minWeightedErrorRateThreshold, (arg("negatives"), arg("positives"), arg("cost")), "Calculates the threshold that minimizes the error rate, given the input data. An optional parameter 'cost' determines the relative importance between false-accepts and false-rejections. This number should be between 0 and 1 and will be clipped to those extremes. The value to minimize becomes: ER_cost = [cost * FAR] + [(1-cost) * FRR]. The higher the cost, the higher the importance given to *not* making mistakes classifying negatives/noise/impostors.");
 def("min_hter_threshold", &err::minHterThreshold, (arg("negatives"), arg("positives")), "Calculates the min_weighted_error_rate_threshold() when the cost is 0.5.");
 def("roc", &roc, (arg("negatives"), arg("positives"), arg("points")), "Calculates the ROC curve given a set of positive and negative scores and a number of desired points. Returns a two-dimensional blitz::Array of doubles that express the X (FRR) and Y (FAR) coordinates in this order. The points in which the ROC curve are calculated are distributed uniformily in the range [min(negatives, positives), max(negatives, positives)].");
