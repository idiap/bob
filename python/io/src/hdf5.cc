...
hdf5file_make_fromstr(const std::string& filename, const std::string& opmode) {
  ...
  else if (opmode[0] == 'a') mode = io::HDF5File::inout;
  else if (opmode[0] == 'w') mode = io::HDF5File::trunc;
 ...
 */
static boost::shared_ptr<io::HDF5File>
hdf5file_make_readonly(const std::string& filename) {
  return boost::make_shared<io::HDF5File>(filename, io::HDF5File::in);
}
...
void bind_io_hdf5() {
  class_<io::HDF5File, boost::shared_ptr<io::HDF5File>, boost::noncopyable>("HDF5File", "A HDF5File allows users to read and write data from and to files containing standard bob binary coded data in HDF5 format. For an introduction to HDF5, please visit http://www.hdfgroup.org/HDF5.", no_init)
    .def("__init__", make_constructor(hdf5file_make_fromstr, default_call_policies(), (arg("filename"), arg("openmode_string"))), "Opens a new file in one of these supported modes: 'r' (read-only), 'a' (read/write/append), 'w' (read/write/truncate) or 'x' (read/write/exclusive)")
    .def("__init__", make_constructor(hdf5file_make_readonly, default_call_policies(), (arg("filename"))), "Opens a new HDF5File for reading.")
