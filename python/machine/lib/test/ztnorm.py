...
class ZTNormTest(unittest.TestCase):
  ...
  def test02_ztnorm_big(self):
    my_A = bob.io.Array("ztnorm_eval_eval.mat").get()
    my_B = bob.io.Array("ztnorm_znorm_eval.mat").get()
    my_C = bob.io.Array("ztnorm_eval_tnorm.mat").get()
    my_D = bob.io.Array("ztnorm_znorm_tnorm.mat").get()

    ref_scores = bob.io.Array("ztnorm_result.mat").get()
    scores = bob.machine.ztnorm(my_A, my_B, my_C, my_D)
    
    self.assertTrue((abs(scores - ref_scores) < 1e-7).all())
 
# Instantiates our standard main module for unittests
main = bob.helper.unittest_main(ZTNormTest)
