...
def cepstral_features_extraction(obj, rate_wavsample, win_length_ms, win_shift_ms, n_filters, n_ceps, dct_norm, f_min, f_max,
  ...
  for i in range(n_frames):
    ...
    ct.pre_emphasis(f2, win_length, 0.95)
...
class CepsTest(unittest.TestCase):
  ...
  def test_mfcc(self):
    ...
    c = bob.sp.Ceps(rate_wavsample[0], 20, 10, 24, 19, 0., 4000., 2, data_array)
    c.reinit(1.0, False, True, True, True, True, True);
    ct = bob.sp.TestCeps(c)
    A = c.ceps_analysis(len(data_array) )
