...
class ArraysetTest(unittest.TestCase):
  ...
  def test02_initialization_with_files(self):
    ...
    A = bob.io.Arrayset(F('test1.hdf5'), 'r') # use whole file.
    ...
    B = bob.io.Arrayset(bob.io.open(F('test1.hdf5'), 'r'), 1) 

    self.assertEqual ( len(B), 2 )

    # Or limit start and end
    C = bob.io.Arrayset(bob.io.open(F('test1.hdf5'), 'r'), 1, 2) #load only #1
    ...
    D = bob.io.Arrayset(F('test1.hdf5'))
  ...
  def test04_load_and_save (self):
    ...
    for k in range(len(A)):
      self.assertEqual ( A.get(k).filename, F('test1.hdf5') )
