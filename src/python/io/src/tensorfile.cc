 ...
 *
 * @brief Python bindings to io::TensorFile
 */
 ...
 */
static boost::shared_ptr<io::TensorFile>
tensorfile_make_fromstr(const std::string& filename, const std::string& opmode) {
  io::TensorFile::openmode mode = io::_unset;
  for (size_t i=0; i<opmode.size(); ++i) {
    if (opmode[i] == 'r') mode |= io::TensorFile::in;
    else if (opmode[i] == 'w') mode |= io::TensorFile::out;
    else if (opmode[i] == 'a' || opmode[i] == '+') mode |= io::TensorFile::append; 
  ...
  return boost::make_shared<io::TensorFile>(filename, mode);
}
...
tensorfile_make_readable(const std::string& filename) {
  return boost::make_shared<io::TensorFile>(filename, io::_in);
}
...
void bind_io_tensorfile() {
  class_<io::TensorFile, boost::shared_ptr<io::TensorFile>, boost::noncopyable>("TensorFile", "A TensorFile allows users to read and write data from and to files containing standard Torch tensor coded data", no_init)
