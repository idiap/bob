...
void bind_io_video() {
  ...
  class_<io::VideoWriter, boost::shared_ptr<io::VideoWriter>, boost::noncopyable>("VideoWriter",
    ...
    .def("append", (void (io::VideoWriter::*)(const blitz::Array<uint8_t,3>&))&io::VideoWriter::append, (arg("frame")), "Writes a new frame to the file. The frame should be setup as a array with 3 dimensions organized in this way (RGB color-bands, height, width). WARNING: At present time we only support arrays that have C-style storages (if you pass reversed arrays or arrays with Fortran-style storage, the result is undefined).")
    .def("append", (void (io::VideoWriter::*)(const blitz::Array<uint8_t,4>&))&io::VideoWriter::append, (arg("frame")), "Writes a set of frames to the file. The frame set should be setup as a array with 4 dimensions organized in this way: (frame-number, RGB color-bands, height, width). WARNING: At present time we only support arrays that have C-style storages (if you pass reversed arrays or arrays with Fortran-style storage, the result is undefined).")
    ;
}
