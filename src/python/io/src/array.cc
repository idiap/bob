...
#define ARRAY_CAST_DEF(T,N,D) .def(BOOST_PP_STRINGIZE(__cast_ ## N ## _ ## D ## __), (blitz::Array<T,D> (io::Array::*)(void) const)&io::Array::cast<T,D>, (arg("self")), ARRAY_CAST_DOC)
...
#define ARRAY_GET_DEF(T,N,D) .def(BOOST_PP_STRINGIZE(__get_ ## N ## _ ## D ## __), (const blitz::Array<T,D> (io::Array::*)(void) const)&io::Array::get<T,D>, (arg("self")), ARRAY_GET_DOC)
...
void bind_io_array() {
  ...
  typedef const io::Array ConstArray;
  class_<io::Array, boost::shared_ptr<io::Array>, boost::shared_ptr<ConstArray> >("Array", "Dataset Arrays represent pointers to concrete data serialized on a io. You can load or refer to real blitz::Arrays using this type.", init<const std::string&, optional<const std::string&> >((arg("filename"),arg("codecname")=""), "Initializes a new array from an external file. An optional codec may be passed."))
    .def("save", &io::Array::save, array_save_overloads((arg("filename"), arg("codecname")=""), "Saves, renames or re-writes the array into a file. It will save if the array is loaded in memory. It will move if the codec used does not change by the filename does. It will re-write if the codec changes."))
    .def("load", &io::Array::load)
    .add_property("loaded", &io::Array::isLoaded)
    ...
    .add_property("codec", &io::Array::getCodec, "Accesses the codec that decodes and encodes this Array from/to files.")
    .add_property("shape", &get_shape, "Accesses the shape of this array")
    .add_property("elementType", &io::Array::getElementType, "Accesses the element type of this array")
