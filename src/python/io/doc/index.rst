...
.. Index file for the Python Torch::io bindings
...
our examples and how Torch will encode and decode the data.
...
.. code-block:: python
  ...
  an_integer = 5
  a_float = 3.1416
  f = torch.io.HDF5File('example1.hdf5')
...
.. code-block:: python
  ...
  >>> f = torch.io.HDF5File('example1.hdf5', 'r') #read only
...
.. code-block:: python
  ...
  >>> f = torch.io.HDF5File('example2.hdf5')
  >>> f.append('arrayset', torch.core.array.float64_1(range(10),(10,)))
  >>> f.append('arrayset', 2*torch.core.array.float64_1(range(10),(10,)))
  >>> f.append('arrayset', 3*torch.core.array.float64_1(range(10),(10,)))
...
.. code-block:: python
  ...
  >>> t = torch.core.array.load('example2.hdf5')
  >>> t
...
list of arrays can only be serialized and de-serialized from specific formats:
  ...
  * Torch3 (``.bindata``), supporting single or double precision float numbers,
    with the codec ``torch3.arrayset.binary``;
  * Torch5spro beta (``.bin``), supports all element types in |project| and any
    dimensionality, with the codec ``torch.arrayset.binary`` (*deprecated*);
  * Torch5spro alpha (``.tensor``), with the codec ``tensor.arrayset.binary``
    (*deprecated*);
...
.. code-block:: python
  ...
  >>> s = torch.io.Arrayset('example2.hdf5')
  >>> print s
...
.. code-block:: python
  ...
  >>> s = torch.io.Arrayset()
  >>> s.elementType
  libpytorch_io.ElementType.unknown
  >>> len(s)
  0
  >>> s.append(torch.core.array.array(range(5), 'float32'))
  >>> len(s)
  1
  >>> s.elementType
  libpytorch_io.ElementType.float32
...
.. code-block:: python
  ...
  >>> t = torch.core.array.array(range(5), 'float32')
...
.. code-block:: python
  ...
  >>> t = torch.core.array.float32_2(range(50), (10,5))
  >>> s.extend(t, 0)
  >>> len(s)
  18
...
save a complex array inside a |project| (``.hdf5``) file into a Torch3
