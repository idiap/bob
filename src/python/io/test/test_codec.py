...
class TextArrayCodec(torch.io.ArrayCodec):
  ...
  def peek (self, filename):
    ...
    the element types defined in torch.io.ElementType. Shape is
    ...
    return (torch.io.ElementType.uint16, (int(entries[0]),))

  def load (self, filename):
    """This method loads the whole data in the file into memory. You should
    return always a torch.io.Array object."""
...
class CodecTest(unittest.TestCase):
  ...
  def test01_CanRegisterArrayCodec(self):
    ...
    torch.io.ArrayCodecRegistry.addCodec(TextArrayCodec())

    # We can test the correct registration of the codec by making sure we can
    # retrieve it from the registry
    c = torch.io.ArrayCodecRegistry.getCodecByName("example.array.text")
    ...
    self.assertEqual('example.array.text' in
        torch.io.ArrayCodecRegistry.getCodecNames(), True)
    self.assertEqual('.Xtxt' in
        torch.io.ArrayCodecRegistry.getExtensions(), True)
  ...
  def test02_Transcoding(self):
    ...
    text = torch.io.ArrayCodecRegistry.getCodecByName("example.array.text")
    binary = torch.io.ArrayCodecRegistry.getCodecByName("hdf5.array.binary")
  ...
  def test03_Deregistration(self):
    ...
    # static method.
    torch.io.ArrayCodecRegistry.removeCodecByName("example.array.text")
