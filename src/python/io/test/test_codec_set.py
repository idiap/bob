...
class TextArraysetCodec(torch.io.ArraysetCodec):
  ...
  def peek (self, filename):
    ...
    one of the element types defined in torch.io.ElementType. Shape is
    ...
    f.close()
    return (torch.io.ElementType.uint16, (int(info[0]),), int(info[1]))
...
class ArraysetCodecTest(unittest.TestCase):
  ...
  def test01_CanRegisterArraysetCodec(self):
    ...
    torch.io.ArraysetCodecRegistry.addCodec(TextArraysetCodec())

    # We can test the correct registration of the codec by making sure we can
    # retrieve it from the registry
    c = torch.io.ArraysetCodecRegistry.getCodecByName("example.arrayset.text")
    ...
    self.assertEqual('example.arrayset.text' in
        torch.io.ArraysetCodecRegistry.getCodecNames(), True)
    self.assertEqual('.XXtxt' in
        torch.io.ArraysetCodecRegistry.getExtensions(), True)

  def test02_Transcoding(self):
    ...
    text = torch.io.ArraysetCodecRegistry.getCodecByName("example.arrayset.text")
    binary = torch.io.ArraysetCodecRegistry.getCodecByName("hdf5.arrayset.binary")
  ...
  def test03_Deregistration(self):

    # You can deregister one or many codecs using the removeCodecByName()
    # static method.
    torch.io.ArraysetCodecRegistry.removeCodecByName("example.arrayset.text")

    self.assertEqual('example.array.text' not in
        torch.io.ArraysetCodecRegistry.getCodecNames(), True)
    self.assertEqual('.XXtxt' not in
        torch.io.ArraysetCodecRegistry.getExtensions(), True)
