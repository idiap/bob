...
void bind_machine_svm() {
  ...
  class_<mach::SupportVector>("SupportVector", "This class can load and run an SVM generated by libsvm. Libsvm is a simple, easy-to-use, and efficient software for SVM classification and regression. It solves C-SVM classification, nu-SVM classification, one-class-SVM, epsilon-SVM regression, and nu-SVM regression. It also provides an automatic model selection tool for C-SVM classification. This document explains the use of libsvm.", no_init)
    .def(init<const char*>((arg("filename")), "Initializes this SVM with the name of a file containing a SVM model as saved by libsvm"))
    .add_property("shape", &get_shape, "Expected input and output sizes. Note the output size is always 1.") 
    .add_property("labels", &labels, "The labels this machine is prepared to cope with")
    .add_property("machine_type", &mach::SupportVector::machineType, "The type of SVM machine contained")
    .add_property("kernel_type", &mach::SupportVector::kernelType, "The type of kernel used by the support vectors in this machine")
    .add_property("polynomialDegree", &mach::SupportVector::polynomialDegree, "The polinomial degree, only valid if the kernel is polynomial")
    .add_property("gamma", &mach::SupportVector::gamma, "The gamma parameter for polynomial, RBF (gaussian) or sigmoidal kernels")
    .add_property("coefficient0", &mach::SupportVector::coefficient0, "The coefficient 0 for polynomial or sigmoidal kernels")
    .add_property("supportsProbability", &mach::SupportVector::supportsProbability, "true if this machine supports probability outputs")
    ...
    .def("save", &mach::SupportVector::save, (arg("self"), arg("filename")), "Saves the currently loaded model to an output file. Overwrites the file, if necessary")
    ;
