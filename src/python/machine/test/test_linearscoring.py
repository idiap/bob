...
class LinearScoringTest(unittest.TestCase):
  ...
  def test01_LinearScoring(self):
    ...
    scores = bob.machine.linearScoring([model1, model2], ubm, [stats1, stats2, stats3], None, True)
  ...
  def test02_LinearScoring_NEW(self):
    ...
    scores = bob.machine.linearScoring2([model1.meanSupervector, model2.meanSupervector], ubm.meanSupervector, ubm.varianceSupervector, [stats1, stats2, stats3], test_channeloffset, True)
    ...
    scores = bob.machine.linearScoring2([model1.meanSupervector, model2.meanSupervector], ubm.meanSupervector, ubm.varianceSupervector, [stats1, stats2, stats3])
    ref_scores = numpy.array([[2372.9, 5207.7, 5275.7], [2215.7, 4868.1, 4932.1]], 'float64')
    self.assertTrue((abs(scores - ref_scores) < 1e-7).all())

    scores = bob.machine.linearScoring2([model1, model2], ubm, [stats1, stats2, stats3])
    ref_scores = numpy.array([[2372.9, 5207.7, 5275.7], [2215.7, 4868.1, 4932.1]], 'float64')
    self.assertTrue((abs(scores - ref_scores) < 1e-7).all())

    # with normalisation
    scores = bob.machine.linearScoring2([model1.meanSupervector, model2.meanSupervector], ubm.meanSupervector, ubm.varianceSupervector, [stats1, stats2, stats3], True)
    ...
    scores = bob.machine.linearScoring2([model1, model2], ubm, [stats1, stats2, stats3], True)
    ref_scores = numpy.array( [[790.9666666666667, 743.9571428571428, 753.6714285714285], [738.5666666666667, 695.4428571428572, 704.5857142857144]], 'float64')
    self.assertTrue((abs(scores - ref_scores) < 1e-7).all())
