...
done using a development set (also called validation set). Nevertheless, the
optimal threshold can be different depending on the relative importance given
to the FAR and the FRR. Hence, in the EPC framework, the cost :math:`\beta \in
[0;1]` is defined as the tradeoff between FAR and FRR. The optimal threshold
:math:`\tau^*` is then computed using different values of :math:`\beta`,
...
.. note::
  ...
  refered as the **positives** represents the client or signal response of the
  classifier. 

  The input to create these two vectors is generated by experiments conducted
  by the user and normally sits in files that need some parsing before these
