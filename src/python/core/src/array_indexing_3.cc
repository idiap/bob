 ...
 * @date Fri 11 Mar 12:12:33 2011 
 *
 * @brief Implements specificities of 3-D array indexing
 */
...
(blitz::Array<T,3>& a, bp::tuple index, bp::object value=bp::object()) {
  if (bp::len(index) != 3) {
    boost::format msg("Trying to index blitz::Array<T,3> using a tuple with %d entries");
    msg % bp::len(index);
  ...
  if (i1c.check()) {
    ...
    if (i2c.check()) {
      ...
      if (i3c.check()) {
        int r3 = i3c();
        r3 = tp::check_range(2, r3, a.lbound(2), a.extent(2));
        if (value.ptr() != Py_None) a(r1,r2,r3) = bp::extract<T>(value);
        return bp::object(a(r1,r2,r3));
      }
    }
  }

  //if you get to this point, have to raise
  boost::format msg("Cannot index blitz::Array<T,3> with input arguments");
...
template <typename T>
static bp::object getitem (blitz::Array<T,3>& a, bp::tuple index) {
  return getset(a, index);
}

template <typename T> static bp::object setitem 
(blitz::Array<T,3>& a, bp::tuple index, bp::object value) {
  return getset(a, index, value);
}

template <typename T>
static void bind_indexing_3(tp::array<T,3>& array) {
  array.object()->def("fill", &tp::fill<T,3>, (bp::arg("self"), bp::arg("other")), "Sets the contents of the current array to be the same as the other array. A full size check is a precondition and I'll raise an exception if the destination sizes and source sizes do not properly match.");
...
void bind_array_indexing_3 () {
  bind_indexing_3(tp::bool_3);
  bind_indexing_3(tp::int8_3);
  bind_indexing_3(tp::int16_3);
  bind_indexing_3(tp::int32_3);
  bind_indexing_3(tp::int64_3);
  bind_indexing_3(tp::uint8_3);
  bind_indexing_3(tp::uint16_3);
  bind_indexing_3(tp::uint32_3);
  bind_indexing_3(tp::uint64_3);
  bind_indexing_3(tp::float32_3);
  bind_indexing_3(tp::float64_3);
  //bind_indexing_3(tp::float128_3);
  bind_indexing_3(tp::complex64_3);
  bind_indexing_3(tp::complex128_3);
  //bind_indexing_3(tp::complex256_3);
}
