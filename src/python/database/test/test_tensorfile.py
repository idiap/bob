...
class TensorFileTest(unittest.TestCase):
  """Performs various tests for the Torch::database::TensorFile types."""
  ...
  def test01_CanCreate(self):
    ...
    outfile = torch.database.TensorFile(tmpname, 'w')
    ...
    self.assertEqual(outfile.elementType, torch.database.ElementType.float64)
    del outfile
    ...
    infile = torch.database.TensorFile(tmpname, 'r')
    self.assertEqual(len(infile), N)
    self.assertEqual(infile.shape, SHAPE)
    self.assertEqual(infile.elementType, torch.database.ElementType.float64)
    for k in range(N): 
    ...
    inoutfile = torch.database.TensorFile(tmpname, 'rw') 
    self.assertEqual(len(inoutfile), N)
    self.assertEqual(inoutfile.shape, SHAPE)
    self.assertEqual(inoutfile.elementType, torch.database.ElementType.float64)
    for k in range(N): 
