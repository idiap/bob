...
class BinaryFileTest(unittest.TestCase):
  """Performs various tests for the Torch::database::BinI/O*File types."""
  ...
  def test01_CanCreate(self):
    ...
    outfile = torch.database.BinFile(tmpname, 'w')
    ...
    self.assertEqual(outfile.elementType, torch.database.ElementType.complex128)
    del outfile
    ...
    infile = torch.database.BinFile(tmpname, 'r')
    ...
    inoutfile = torch.database.BinFile(tmpname, 'rw') 
