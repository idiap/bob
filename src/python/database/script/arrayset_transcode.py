...
def print_codecs():
  ...
  print "--------------------------------+------------------------------"
  for k in torch.database.ArraysetCodecRegistry.getCodecNames():
    codec = torch.database.ArraysetCodecRegistry.getCodecByName(k)
...
torch.database.arrayset_transcode(sys.argv[1], sys.argv[2])
