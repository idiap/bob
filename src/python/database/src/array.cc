...
#define ARRAY_COPY_DEF(T,D) .def("bzcopy", &Tc::Array::copy<T,D>, (arg("self"), arg("array")), ARRAY_COPY_DOC)
...
#define ARRAY_REFER_DEF(T,N,D) .def(BOOST_PP_STRINGIZE(refer_ ## N ## _ ## D), (blitz::Array<T,D> (Tc::Array::*)(void))&Tc::Array::refer<T,D>, (arg("self")), ARRAY_REFER_DOC)

static const char* get_filename(Tc::Array& a) {
  return a.getFilename().c_str();
}

static void set_filename(Tc::Array& a, const char* filename) {
  std::string f(filename);
  a.setFilename(f);
}

void bind_database_array() {
  class_<Tc::Array, boost::shared_ptr<Tc::Array> >("Array", "Dataset Arrays represent pointers to concrete data serialized on a database. You can load or refer to real blitz::Arrays using this type.", init<const Tc::Arrayset&,const size_t,optional<const std::string&,const Tc::LoaderType> >((arg("parent"),arg("id"),arg("filename"),arg("loader")), "Initializes a new array given its parent"))
//    .add_property("loaded", &Tc::Array::getIsLoaded, &Tc::Array::setIsLoaded)
    ...
    .def("getParentArrayset", &Tc::Array::getParentArrayset, "Access the parent Arrayset for this array. The Arrayset contains global properties you may be interested at such as array types and dimensions.", return_internal_reference<>())
