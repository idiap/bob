/**
 * @file cxx/ip/src/ipLBPTopOperator.cc
...
bob::ipLBPTopOperator::ipLBPTopOperator(int radius_xy, 
                                          int points_xy, 
                                          int radius_xt, 
                                          int points_xt, 
                                          int radius_yt,
 ...
 */
static bob::ipLBP* make_lbp(int radius, int points)
{
  bob::ipLBP* retval = 0;
  if (points != 4 && points != 8) {
    ...
    if (points == 4) retval = new bob::ipLBP4R(radius);
    else retval = new bob::ipLBP8R(radius);
...
void bob::ipLBPTopOperator::optionChanged(const char* /* name */)
{
...
bob::ipLBPTopOperator::~ipLBPTopOperator() {
  delete m_lbp_xy;
  m_lbp_xy = 0;
  delete m_lbp_xt;
  m_lbp_xt = 0;
...
bool bob::ipLBPTopOperator::process(const bob::ShortTensor& tensor,
    bob::Image& xy, bob::Image& xt, bob::Image& yt) const
{
  ...
  if (tensor.size(3) != 1) {
    bob::warning("Cannot work with colored images, please use grayscale");
    return false;
  }
