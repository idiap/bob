 ...
 */
  namespace ip {
    ...
    void geoNorm(const blitz::Array<T,2>& src, blitz::Array<T,2>& dst, 
		 ...
		 const int right_h, const int right_h,
		 const int target_distance_between_left_and_right,
		 const int final_height, const int final_width)
    {
	    //shift to center
	    blitz::Array<T,2> dst_cnt = sameAs(src);
	    torch::ip::shiftToCenterOfPoints(src, dst_cnt, left_h, left_w, right_h, right_w);
	    
	    // rotate
	    double angle = torch::ip::getRotateAngleToLevelOutHorizontal(left_h, left_w, right_h, right_w);
	    blitz::Array<T,2> dst_rot(torch::ip::getShapeRotated(dst_cnt, angle));
	    torch::ip::rotate(dst_cnt, dst_rot, angle);
	    ...
	    torch::ip::scale(dst_rot, dst_scl, new_height, new_width);
