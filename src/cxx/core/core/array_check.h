 ...
 */
  namespace core { namespace array {
    ...
    void contCopy(const std::map<K, blitz::Array<T,D> >& src,
               std::map<K, blitz::Array<T,D> >& dst)
    {
      for(typename std::map<K, blitz::Array<T,D> >::const_iterator 
            it=src.begin(); it!=src.end(); ++it)
      {
        // A little more tricky due to the 'pass by reference' in the
        // std::pair constructor. This means that the blitz reference counter 
        // is not incremented.
        dst[it->first] = blitz::Array<T,D>();
        dst[it->first].resize(it->second.shape());
