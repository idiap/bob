  ...
  namespace core {
      ...
      switch(m_header.m_type)
      {
        ...
        case array::t_float128:
          for( size_t i=0; i<m_header.m_n_elements; ++i) {
            ...
            m_stream.write( reinterpret_cast<const char*>(&ld), 
              sizeof(long double));
          }
          break;
        ...
        case array::t_complex256:
          for( size_t i=0; i<m_header.m_n_elements; ++i) {
            ...
            m_stream.write( reinterpret_cast<const char*>(&cld), 
    ...
    BinFile& BinFile::readWithCast(T* multiarray) {
      ...
      switch(m_header.m_type)
      {
        ...
        case array::t_float128:
          for( size_t i=0; i<m_header.m_n_elements; ++i) {
            m_stream.read( reinterpret_cast<char*>(&ld), 
              sizeof(long double));
        ...
        case array::t_complex256:
          for( size_t i=0; i<m_header.m_n_elements; ++i) {
            m_stream.read( reinterpret_cast<char*>(&cld), 
              sizeof(std::complex<long double>));
    ...
    template <int d>
    void BinFile::initTypeHeader(const blitz::Array<long double,d>& bl)
    {
