...
namespace h5 = Torch::database::detail::hdf5;
...
static boost::shared_ptr<hid_t> open_dataset(boost::shared_ptr<h5::File>& file,
  ...
  *retval = H5Dopen2(*file->m_id, path.c_str(), H5P_DEFAULT);
  if (*retval < 0) {
    throw db::HDF5StatusError("H5Dopen2", *retval);
  }
  return retval;
}
...
static boost::shared_ptr<hid_t> open_datatype(boost::shared_ptr<h5::File>& file,
  ...
  *retval = H5Dget_type(*ds);
  if (*retval < 0) {
    throw db::HDF5StatusError("H5Dget_type", *retval);
  }
  return retval;
}
...
static boost::shared_ptr<hid_t> open_plist(hid_t classid) {
  ...
  *retval = H5Pcreate(classid);
  if (*retval < 0) {
    throw db::HDF5StatusError("H5Pcreate", *retval);
  }
  return retval;
}
...
(boost::shared_ptr<h5::File>& file, const std::string& path,
  ...
  *retval = H5Dget_space(*ds);
  if (*retval < 0) throw db::HDF5StatusError("H5Dget_space", *retval);
  return retval;
}

static boost::shared_ptr<hid_t> open_memspace(const db::HDF5Type& t) {
  boost::shared_ptr<hid_t> retval(new hid_t(-1), std::ptr_fun(delete_h5dataspace));
  *retval = H5Screate_simple(t.shape().n(), t.shape().get(), 0);
  if (*retval < 0) throw db::HDF5StatusError("H5Screate_simple", *retval);
  return retval;
}
...
static bool is_chunked(boost::shared_ptr<hid_t>& dataset) {
  ...
  if (plist < 0) throw db::HDF5StatusError("H5Pget_create_plist", plist);
 ...
 */
static db::HDF5Shape get_extents(boost::shared_ptr<hid_t>& space) {
  int rank = H5Sget_simple_extent_ndims(*space);
  if (rank < 0) throw db::HDF5StatusError("H5Sget_simple_extent_ndims", rank);
  //is at least a list of scalars, but could be a list of arrays
  db::HDF5Shape shape(rank);
  herr_t status = H5Sget_simple_extent_dims(*space, shape.get(), 0);
  if (status < 0) throw db::HDF5StatusError("H5Sget_simple_extent_dims",status);
  return shape;
}
...
static void create_dataset (boost::shared_ptr<h5::File>& file, 
 const std::string& path, const db::HDF5Type& type,
 size_t compression) {

  db::HDF5Shape xshape(type.shape().n()+1);
  for (size_t i=0; i<type.shape().n(); ++i) xshape[i+1] = type.shape()[i];
  xshape[0] = 0; ///< no elements for the time being

  db::HDF5Shape maxshape(xshape);
  ...
  *space = H5Screate_simple(type.shape().n()+1, xshape.get(), 
      maxshape.get());
  if (*space < 0) throw db::HDF5StatusError("H5Screate_simple", *space);
  ...
  if (status < 0) 
    throw db::HDF5StatusError("H5Pset_create_intermediate_group", status);
  ...
  *dataset = H5Dcreate2(*file->m_id, path.c_str(),
  ...
  if (*dataset < 0) throw db::HDF5StatusError("H5Dcreate2", *dataset);
}
  ...
  m_type(type)
{
  ...
  m_select_count = m_extent;
  m_select_count[0] = 1;
  m_type = db::HDF5Type(m_dt, m_extent);
  m_memspace = open_memspace(m_type);
  m_chunked = is_chunked(m_id);
}
...
void h5::Dataset::select (size_t index) {
  ...
  if (status < 0) throw db::HDF5StatusError("H5Sselect_hyperslab", status);
}
...
void h5::Dataset::read (void* buffer) {
  ...
  herr_t status = H5Dread(*m_id, *htype, *m_memspace, *m_filespace,
      H5P_DEFAULT, buffer);
  if (status < 0) throw db::HDF5StatusError("H5Dread", status);
}
...
void h5::Dataset::extend () {
  ...
  if (status < 0) {
    m_extent[0] -= 1;
    throw db::HDF5StatusError("H5Dset_extent", status);
  }
...
void h5::Dataset::write (const void* buffer) {
  ...
  herr_t status = H5Dwrite(*m_id, *htype, *m_memspace, *m_filespace,
      H5P_DEFAULT, buffer);
  if (status < 0) throw db::HDF5StatusError("H5Dwrite", status);
}
...
void h5::File::unlink (const std::string& path) {
  ...
  if (status < 0) throw db::HDF5StatusError("H5Ldelete", status);
...
void h5::File::rename (const std::string& from,
  ...
  if (status < 0) throw db::HDF5StatusError("H5Lmove", status);
...
static herr_t fill_index_callback(hid_t group, const char *name, 
  ...
  if (status < 0) throw db::HDF5StatusError("H5Oget_info_by_name", status);
...
void h5::index(boost::shared_ptr<h5::File>& file,
  ...
  if (status < 0) throw db::HDF5StatusError("H5Lvisit_by_name", status);
}
