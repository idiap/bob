...
#define TORCH_DATABASE_HDF5UTILS_H
...
namespace Torch { namespace database { namespace detail { namespace hdf5 {
  ...
  class Dataset {
    ...
    public:
      ...
      Dataset(boost::shared_ptr<File>& f, const std::string&, 
          const Torch::database::HDF5Type& type);
      ...
      template <typename T> void read(size_t index, T& value) {
        if (index >= m_extent[0])
          throw Torch::database::HDF5IndexError(m_parent->m_path.string(), 
              m_path, m_extent[0], index);
        if (m_type != Torch::database::HDF5Type(value))
          throw Torch::database::HDF5IncompatibleIO(m_parent->m_path.string(), 
        ...
        void readArray(size_t index, blitz::Array<T,N>& value) {
          if (index >= m_extent[0])
            throw Torch::database::HDF5IndexError(m_parent->m_path.string(), 
                m_path, m_extent[0], index);
          if (m_type != Torch::database::HDF5Type(value))
            throw Torch::database::HDF5IncompatibleIO
              (m_parent->m_path.string(), m_path, m_type.str(),
               Torch::database::HDF5Type(value).str());
      ...
      template <typename T> void add(const T& value) {
        if (!m_chunked)
          throw Torch::database::HDF5NotExpandible(m_parent->m_path.string(), 
              m_path);
        if (m_type != Torch::database::HDF5Type(value))
          throw Torch::database::HDF5IncompatibleIO(m_parent->m_path.string(), 
              m_path, m_type.str(), Torch::database::HDF5Type(value).str());
    ...
    public: //representation
      ...
      Torch::database::HDF5Shape m_extent; ///< the actual shape of this set
      Torch::database::HDF5Shape m_select_offset; ///< hyperslab offset
      Torch::database::HDF5Shape m_select_count; ///< hyperslab count
      bool m_chunked; ///< true if this dataset is expandible.
      Torch::database::HDF5Type m_type; ///< the type information for this set
...
#endif /* TORCH_DATABASE_HDF5UTILS_H */
