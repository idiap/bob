 ...
 *
 * @brief Converts data from the Portable Bitmap (PBM) P4 image format into
 * Torch5spro.
 */

#ifndef TORCH5SPRO_DATABASE_PBMIMAGE_ARRAYCODEC_H 
#define TORCH5SPRO_DATABASE_PBMIMAGE_ARRAYCODEC_H 1
...
namespace Torch { namespace database {
  ...
  /**
   * Reads and writes single Arrays to PBM P4 image files
   */
  class PBMImageArrayCodec : public ArrayCodec {

    public:

      PBMImageArrayCodec();

      virtual ~PBMImageArrayCodec();
...
`       * Parses the header of the file and return the width and the height
        * An exception is thrown if the header is not in PBM format.
        */
      void 
