...
#ifndef TORCH_IO_INLINEDARRAYIMPL_H
#define TORCH_IO_INLINEDARRAYIMPL_H
...
namespace Torch { namespace io { namespace detail {
  ...
  template<typename T, int D> const blitz::Array<T,D>& InlinedArrayImpl::get() const {
    if (m_elementtype != Torch::core::array::getElementType<T>()) {
      throw Torch::io::TypeError(Torch::core::array::getElementType<T>(), m_elementtype);
    }
    if (m_ndim != D) {
      throw Torch::io::DimensionError(D, m_ndim);
    }
...
#endif /* TORCH_IO_INLINEDARRAYIMPL_H */
