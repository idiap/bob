...
#ifndef TORCH_IO_ARRAYSETCODECREGISTRY_H 
#define TORCH_IO_ARRAYSETCODECREGISTRY_H
...
namespace Torch { namespace io {
  ...
  class ArraysetCodecRegistry {
    ...
    public:
      ...
      static void addCodec(boost::shared_ptr<Torch::io::ArraysetCodec> codec);
      static void removeCodecByName(const std::string& codecname);

      static boost::shared_ptr<const Torch::io::ArraysetCodec>
      ...
      static void getCodecNames(T& t) {
        ...
        for (std::map<std::string, boost::shared_ptr<Torch::io::ArraysetCodec> >::const_iterator it = ptr->s_name2codec.begin(); it != ptr->s_name2codec.end(); ++it) t.push_back(it->first);
      }

      template <typename T>
      static void getExtensions(T& t) {
        boost::shared_ptr<ArraysetCodecRegistry> ptr = instance();
        for (std::map<std::string, boost::shared_ptr<Torch::io::ArraysetCodec> >::const_iterator it = ptr->s_extension2codec.begin(); it != ptr->s_extension2codec.end(); ++it) t.push_back(it->first);
      }
    ...
    private:
      ...
      std::map<std::string, boost::shared_ptr<Torch::io::ArraysetCodec> > s_name2codec;
      std::map<std::string, boost::shared_ptr<Torch::io::ArraysetCodec> > s_extension2codec;
    
  };

}}

#endif /* TORCH_IO_ARRAYSETCODECREGISTRY_H */
