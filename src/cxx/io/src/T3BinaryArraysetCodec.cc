...
#include "io/T3BinaryArraysetCodec.h"
...
static bool register_codec() {
  io::ArraysetCodecRegistry::addCodec(boost::shared_ptr<io::ArraysetCodec>(new io::T3BinaryArraysetCodec())); 
  return true;
}

static bool codec_registered = register_codec(); 

io::T3BinaryArraysetCodec::T3BinaryArraysetCodec()
...
void io::T3BinaryArraysetCodec::peek(const std::string& filename, 
...
io::detail::InlinedArraysetImpl io::T3BinaryArraysetCodec::load
...
io::Array io::T3BinaryArraysetCodec::load
...
void io::T3BinaryArraysetCodec::append
(const std::string& filename, const Array& array) const {
  ...
  if (boost::filesystem::exists(filename)) { //the new array must conform!
    ...
    if (array.getNDim() != ndim) throw io::DimensionError(array.getNDim(), ndim);
    if (shape[0] != array.getShape()[0]) throw io::DimensionError(array.getShape()[0], shape[0]);
    if (array.getElementType() != eltype) throw io::TypeError(array.getElementType(), eltype);
...
void io::T3BinaryArraysetCodec::save (const std::string& filename, 
    const detail::InlinedArraysetImpl& data) const {
  //can only save uni-dimensional data, so throw if that is not the case
  if (data.getNDim() != 1) throw io::DimensionError(data.getNDim(), 1);
