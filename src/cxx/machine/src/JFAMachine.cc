...
mach::JFABaseMachine::JFABaseMachine(const mach::JFABaseMachine& other):
  ...
  m_U(ca::ccopy(other.m_U)),
  m_V(ca::ccopy(other.m_V)),
  m_d(ca::ccopy(other.m_d))
{
}
...
mach::JFAMachine::JFAMachine(const mach::JFAMachine& other):
  ...
  m_y(ca::ccopy(other.m_y)),
  m_z(ca::ccopy(other.m_z)),
  m_y_for_x(ca::ccopy(other.m_y_for_x)),
  m_z_for_x(ca::ccopy(other.m_z_for_x)),
...
void mach::JFAMachine::computeIdPlusUProd(const blitz::Array<double,1>& N, const blitz::Array<double,1>& F) 
{
  ...
  math::inv(m_tmp_ruru, m_cache_IdPlusUProd); // m_cache_IdPlusUProdh = ( I+Ut*diag(sigma)^-1*N*U)^-1
}
...
void mach::JFAMachine::computeFn_x(const blitz::Array<double,1>& N, const blitz::Array<double,1>& F)
{
  ...
  math::prod(V, m_y_for_x, m_tmp_CD_b);
...
void mach::JFAMachine::updateX_fromCache()
{
  ...
  math::prod(m_cache_IdPlusUProd, m_tmp_ru, m_x);
}

void mach::JFAMachine::estimateX(boost::shared_ptr<const mach::GMMStats> gmm_stats) {
  boost::shared_ptr<mach::GMMMachine> ubm(getJFABase()->getUbm());
