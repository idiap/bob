#include "FrontalFace20x20_OpenCV.h"

namespace Torch {

FrontalFace20x20_OpenCV::FrontalFace20x20_OpenCV(int postype_) : FaceModel(postype_)
{
	D_EYES = 7.0;
	// un-biased
	Y_UPPER = 7.0;
...
bool FrontalFace20x20_OpenCV::ldm2bbx()
{
	...
	if(verbose)
		print("FrontalFace20x20_OpenCV: c0x=%g c0y=%g\n", c0x, c0y);
	...
	if(verbose)
		print("FrontalFace20x20_OpenCV: x=%d y=%d w=%d h=%d\n", upperleft_x, upperleft_y, bbx_width, bbx_height);
...
bool FrontalFace20x20_OpenCV::bbx2ldm()
{
...
bool FrontalFace20x20_OpenCV::bbx2ldm(int n_bbx_points_, sPoint2D *bbx_points_, int n_ldm_points_, sPoint2D *ldm_points_)
{
	if(n_ldm_points_ != 2)
	{
		warning("FrontalFace20x20_OpenCV::bbx2ldm() n_ldm_points_ != 2.");
		return false;
	}

	if(n_bbx_points_ != 4)
	{
		warning("FrontalFace20x20_OpenCV::to() n_bbx_points_ != 4.");
		return false;
	}
...
FrontalFace20x20_OpenCV::~FrontalFace20x20_OpenCV()
{
}
