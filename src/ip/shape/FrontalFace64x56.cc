#include "FrontalFace64x56.h"

namespace Torch {

FrontalFace64x56::FrontalFace64x56(int postype_) : FaceModel(postype_)
{
	D_EYES = 33.0;
...
bool FrontalFace64x56::ldm2bbx()
{
	...
	if((int) r_eye.y != (int) l_eye.y)
	   	warning("FrontalFace64x56: y coordinates of eyes should be equal (%d == %d)", (int) r_eye.y, (int) l_eye.y);

	c0x = l_eye.x + EEx / 2.0;
	c0y = l_eye.y + EEy / 2.0;

	if(verbose)
		print("FrontalFace64x56: c0x=%g c0y=%g\n", c0x, c0y);

	double r = 2.0 * PUPIL_SE / D_EYES;
	double en_snls = EN_GN - (SN_GN - 0.5 * SN_LS);
	double y_lower = en_snls / r;
	double y_upper = 56 - y_lower;

	if(verbose)
		print("FrontalFace64x56: upper=%d lower=%d\n", (int) y_upper, (int) y_lower);
...
bool FrontalFace64x56::bbx2ldm()
{
	warning("FrontalFace64x56::bbx2ldm() not implemented.");
	return false;
}

bool FrontalFace64x56::bbx2ldm(int n_bbx_points_, sPoint2D *bbx_points_, int n_ldm_points_, sPoint2D *ldm_points_)
{
	warning("FrontalFace64x56::bbx2ldm(...) not implemented.");
	return false;
}

FrontalFace64x56::~FrontalFace64x56()
{
}
