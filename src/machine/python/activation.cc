...
void bind_machine_activation() {
  class_<bob::machine::Activation, boost::shared_ptr<bob::machine::Activation>, boost::noncopyable>("Activation", 
    ...
    .def("f", &bob::machine::Activation::f, (arg("z")),
        "Computes activated value, given an input ``z``")
    .def("__call__", &bob::machine::Activation::f, (arg("z")), 
        "Computes activated value, given an input ``z``")
    ...
    .def("save", &bob::machine::Activation::save, (arg("h5f")), 
       "Saves itself to a :py:class:`bob.io.HDF5File`")
    .def("load", &bob::machine::Activation::load, (arg("h5f")), 
       "Loads itself from a :py:class:`bob.io.HDF5File`")
    .def("unique_identifier", 
        &bob::machine::Activation::unique_identifier,
  ...
  class_<bob::machine::IdentityActivation, boost::shared_ptr<bob::machine::IdentityActivation>, bases<bob::machine::Activation> >("IdentityActivation", "Computes :math:`f(z) = z` as activation function", init<>())
    ;
  class_<bob::machine::LinearActivation, boost::shared_ptr<bob::machine::LinearActivation>, bases<bob::machine::Activation> >("LinearActivation", "Computes :math:`f(z) = Cz` as activation function", init<optional<double> >((arg("C")=1.), "Builds a new linear activation function with a given constant. Don't use this if you just want to set constant to the default value (1.0). In such a case, prefer to use the more efficient :py:class:`bob.machine.IdentityActivation`."))
    ;
  class_<bob::machine::HyperbolicTangentActivation, boost::shared_ptr<bob::machine::HyperbolicTangentActivation>, bases<bob::machine::Activation> >("HyperbolicTangentActivation", "Computes :math:`f(z) = tanh(z)` as activation function", init<>())
    ;
  class_<bob::machine::MultipliedHyperbolicTangentActivation, boost::shared_ptr<bob::machine::MultipliedHyperbolicTangentActivation>, bases<bob::machine::Activation> >("MultipliedHyperbolicTangentActivation", "Computes :math:`f(z) = Ctanh(Mz)` as activation function", init<optional<double, double> >((arg("C")=1., arg("M")=1.), "Builds a new hyperbolic tangent activation fucntion with a given constant for the inner and outter products. Don't use this if you just want to set the constants to the default values (1.0). In such a case, prefer to use the more efficient :py:class:`bob.machine.HyperbolicTangentActivation`."))
    ;
  class_<bob::machine::LogisticActivation, boost::shared_ptr<bob::machine::LogisticActivation>, bases<bob::machine::Activation> >("LogisticActivation", "Computes the so-called logistic function: :math:`f(z) = 1. / (1. + e^{-z})` as activation function", init<>())
    ;
}
