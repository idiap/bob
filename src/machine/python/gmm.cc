...
void bind_machine_gmm()
{
  ...
  class_<bob::machine::GMMMachine, boost::shared_ptr<bob::machine::GMMMachine>, bases<bob::machine::Machine<blitz::Array<double,1>, double> > >("GMMMachine",
    ...
    .def(self == self)
    .def(self != self)
    .def("is_similar_to", &bob::machine::GMMMachine::is_similar_to, (arg("self"), arg("other"), arg("epsilon") = 1e-8), "Compares this GMMMachine with the 'other' one to be approximately the same; each parameter might differ maximal with the given epsilon.")
