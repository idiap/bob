...
void bind_machine_linear() {
  class_<bob::machine::LinearMachine, boost::shared_ptr<bob::machine::LinearMachine>
    ...
    .def(init<bob::io::HDF5File&>((arg("config")), "Constructs a new LinearMachine from a configuration file. Both weights and biases have their dimensionalities checked between each other for consistency."))
