...
namespace Torch {
  ...
  double computeEER(LabelledMeasure* measures, int n, double* frr, 
      double* far, int number_of_positives_, bool sort)
  {
    if(sort) 
      qsort(measures, n, sizeof(LabelledMeasure), cmp_labelledmeasure);
  ...
  double computeHTER(LabelledMeasure* measures, int n, double* frr, 
      double* far, int number_of_positives_, bool sort, float ratio_far)
  {
    if(sort) 
      qsort(measures, n, sizeof(LabelledMeasure), cmp_labelledmeasure);
  ...
  void computeFaFr(real thrd, Int_real* to_sort, int n, real* frr, 
      real* far, int number_of_clients_,bool sort)
  {
    //sort the scores
    if(sort)
    ...
    {
      for(int i=0;i<n;i++)
        if(to_sort[i].the_int == 1) number_of_clients++;
    } 
    else number_of_clients = number_of_clients_;
