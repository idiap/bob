...
// Declaration of functions from FFTPACK4.1
...
bool spDCT_pack41::processInput(const Tensor& input)
{
	if (input.nDimension() == 1)
	{
      ...
      // Allocate C arrays according to the fftpack requirements 
      //    (slightly larger)
      double *x = new double[N];

			// Copy the double tensor in the C array
			for(int i=0; i < N; ++i) x[i] = (*R)(i);

      // Compute the DCT
      cosqb_( &N, x, wsave);

			// Update the array with correct coefficients
