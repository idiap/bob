/**
 * @file sp/cxx/test/fftkiss.cc
 * @date Thu Nov 14 12:24:57 CET 2013
 * @author Laurent El Shafey <Laurent.El-Shafey@idiap.ch>
 *
 * @brief Compare FFT and FCT based on kissfft with naive DFT DCT
 * implementations.
 *
...
#define BOOST_TEST_MODULE sp-FCT_FFT-KISS Tests
...
void test_fct1D( const blitz::Array<double,1> t, double eps) 
{
  ...
  bob::sp::DCT1DKiss fct(t.extent(0));
  fct(t, t_fct);
  ...
  bob::sp::IDCT1DKiss ifct(t.extent(0));
...
void test_fct2D( const blitz::Array<double,2> t, double eps) 
{
  ...
  bob::sp::DCT2DKiss fct(t.extent(0), t.extent(1));
  fct(t, t_fct);
  BOOST_REQUIRE_EQUAL(t_fct.extent(0), t.extent(0));
  ...
  bob::sp::IDCT2DKiss ifct(t.extent(0), t.extent(1));
  ifct(t_fct, t_fct_ifct);
  BOOST_REQUIRE_EQUAL(t_fct_ifct.extent(0), t.extent(0));
...
void test_fft1D( const blitz::Array<std::complex<double>,1> t, double eps) 
{
  ...
  bob::sp::FFT1DKiss fft(t.extent(0));
  fft(t, t_fft);
  ...
  bob::sp::IFFT1DKiss ifft(t.extent(0));
...
void test_fft2D( const blitz::Array<std::complex<double>,2> t, double eps) 
{
  ...
  bob::sp::FFT2DKiss fft(t.extent(0), t.extent(1));
  fft(t, t_fft);
  BOOST_REQUIRE_EQUAL(t_fft.extent(0), t.extent(0));
  ...
  bob::sp::IFFT2DKiss ifft(t.extent(0), t.extent(1));
  ifft(t_fft, t_fft_ifft);
  BOOST_REQUIRE_EQUAL(t_fft_ifft.extent(0), t.extent(0));
