  ...
  blitz::Array<double,1>& eigen_values, const blitz::Array<double,2>& ar)
{
  ...
  // the number of generated projections is at most K - 1 where K is the
  // minimum between the dimensions of the data matrix 'ar', simply because the
  // rank of the covariance matrix generated by 'ar' is, by definition, K - 1
  const int n_sigma = (int)std::min(n_features, n_samples) - 1;
  ...
  if (n_outputs != (size_t)n_sigma) {
    boost::format m("Number of outputs (%d) does not match min(#features, #samples)-1 = min(%d, %d)-1 = %d");
   ...
   */
  blitz::Array<double,2> U(n_features, n_sigma+1);
  blitz::Array<double,1> sigma(n_sigma+1);
