...
(boost::shared_ptr<bob::io::detail::hdf5::Group>& par, const std::string& name) {
...
static void reset_compatibility_list(boost::shared_ptr<hid_t>& space,
  ...
  switch (file_base.shape().n()) {
    ...
    case 1: ///< file type has 1 dimension
      descr.push_back(bob::io::HDF5Descriptor(file_base.type(),
    ...
    case 5:
      {
        bob::io::HDF5Shape alt = file_base.shape();
        alt <<= 1; ///< contract shape
        descr.push_back(bob::io::HDF5Descriptor(bob::io::HDF5Type(file_base.type(), alt),
  ...
  descr.push_back(bob::io::HDF5Descriptor(file_base, 1, false));
}
 ...
 */
static void create_dataset (boost::shared_ptr<bob::io::detail::hdf5::Group> par,
 const std::string& name, const bob::io::HDF5Type& type, bool list,
 size_t compression) {
  ...
  bob::io::HDF5Shape xshape(type.shape());
  ...
  bob::io::HDF5Shape maxshape(xshape);
  ...
  *space = H5Screate_simple(xshape.n(), xshape.get(), maxshape.get());
  ...
  //array shape.
  bob::io::HDF5Shape chunking(xshape);
  chunking[0] = 1;
 ...
 */
static void create_string_dataset (boost::shared_ptr<bob::io::detail::hdf5::Group> par,
 const std::string& name, const bob::io::HDF5Type& type, size_t compression) {
  ...
  m_memspace()
{
  ...
  bob::io::DefaultHDF5ErrorStack->mute();
  ...
  m_dt = open_datatype(m_id);
  m_filespace = open_filespace(m_id);
  bob::io::HDF5Type file_type(m_dt, get_extents(m_filespace));
...
bob::io::detail::hdf5::Dataset::~Dataset() { }

size_t bob::io::detail::hdf5::Dataset::size () const {
  return m_descr[0].size;
}

size_t bob::io::detail::hdf5::Dataset::size (const bob::io::HDF5Type& type) const {
...
boost::shared_ptr<bob::io::detail::hdf5::Group> bob::io::detail::hdf5::Dataset::parent() {
  return m_parent.lock();
}

const std::string& bob::io::detail::hdf5::Dataset::filename() const {
  return parent()->filename();
}

std::string bob::io::detail::hdf5::Dataset::url() const {
  return filename() + ":" + path();
}

std::string bob::io::detail::hdf5::Dataset::path() const {
  return parent()->path() + "/" + m_name;
}

const boost::shared_ptr<bob::io::detail::hdf5::File> bob::io::detail::hdf5::Dataset::file() const {
  return parent()->file();
}

boost::shared_ptr<bob::io::detail::hdf5::File> bob::io::detail::hdf5::Dataset::file() {
  return parent()->file();
}
 ...
 */
static std::vector<bob::io::HDF5Descriptor>::iterator
  find_type_index(std::vector<bob::io::HDF5Descriptor>& descr,
      const bob::io::HDF5Type& user_type) {
  std::vector<bob::io::HDF5Descriptor>::iterator it = descr.begin();
  for (; it != descr.end(); ++it) {
...
std::vector<bob::io::HDF5Descriptor>::iterator
bob::io::detail::hdf5::Dataset::select (size_t index, const bob::io::HDF5Type& dest) {
...
void bob::io::detail::hdf5::Dataset::read_buffer (size_t index, const bob::io::HDF5Type& dest, void* buffer) {

  std::vector<bob::io::HDF5Descriptor>::iterator it = select(index, dest);
...
void bob::io::detail::hdf5::Dataset::gettype_attribute(const std::string& name,
          bob::io::HDF5Type& type) const {
  bob::io::detail::hdf5::gettype_attribute(m_id, name, type);
}

bool bob::io::detail::hdf5::Dataset::has_attribute(const std::string& name) const {
...
void bob::io::detail::hdf5::Dataset::write_attribute (const std::string& name,
...
template <> void bob::io::detail::hdf5::Dataset::read<std::string>(size_t index, std::string& value) {
...
template <> void bob::io::detail::hdf5::Dataset::replace<std::string>(size_t index, const std::string& value) {
...
template <> std::string bob::io::detail::hdf5::Dataset::get_attribute(const std::string& name) const {
  HDF5Type type;
