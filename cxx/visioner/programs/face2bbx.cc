#include "visioner/models/ipyramid.h"
#include "visioner/util/timer.h"

// Compute the face bbx from the eye coordinates
visioner::rect_t eyes2bbx(const visioner::point_t& leye, const visioner::point_t& reye)
{
  ...
  const double ratio = visioner::my_sqrt(EEx * EEx + EEy * EEy) / D_EYES;

  return visioner::rect_t(c0x - ratio * 0.5 * MODEL_WIDTH, 
      c0y - ratio * Y_UPPER,
...
int main(int argc, char *argv[]) {	
  ...
  if (	po_vm.empty() || po_vm.count("help") || 
      !po_vm.count("input"))
  {
    visioner::log_error("face2bbx") << po_desc << "\n";
    exit(EXIT_FAILURE);
  }
  ...
  {
    visioner::log_error("face2bbx") << "Failed to load the face datasets <" << (cmd_input) << ">!\n";
    exit(EXIT_FAILURE);
  }
  ...
  for (std::size_t i = 0; i < ifiles.size(); i ++)
  {
    ...
    for (visioner::objects_t::iterator it = objects.begin(); it != objects.end(); ++ it)
    {
      visioner::Keypoint leye, reye, eye, ntip;
      ...
      {                                
        const visioner::point_t& eye_pt = eye.m_point;
        const visioner::point_t& ntip_pt = ntip.m_point;

        const visioner::scalar_t dx = visioner::my_abs(eye_pt.x() - ntip_pt.x());
        const visioner::scalar_t width = dx * 2.5;                                

        it->move(eyes2bbx(
              visioner::point_t(eye_pt.x() - 0.25 * width, eye_pt.y()),
              visioner::point_t(eye_pt.x() + 0.25 * width, eye_pt.y())));
      }
      else
      {
        it->move(visioner::rect_t(-1, -1, -1, -1));
      }
    }
