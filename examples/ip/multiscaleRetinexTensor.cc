...
int main(int argc, char* argv[])
{
	...
	CmdLine cmd;
	cmd.info("Perform Multiscale Retinex over a TensorFile and save it to a TensorFile.\n");
	...
	//
	Tensor *tensor = NULL;
	ipSelfQuotientImage msr;

	CHECK_FATAL(msr.setIOption("s_nb", s_nb) == true);
	CHECK_FATAL(msr.setIOption("s_min", s_min) == true);
	CHECK_FATAL(msr.setIOption("s_step", s_step) == true);
	CHECK_FATAL(msr.setDOption("Sigma", sigma) == true);

	ipHistoEqual histoEqual;

	// Loop over each Tensor contained in the TensorFile
	print("\nPerforming Multiscale Retinex over each sample ...\n");
	ShortTensor* out_2d;
	while ((tensor = itf.load()) != 0)
	{
		...
		CHECK_FATAL(msr.process(imagegray) == true);
		CHECK_FATAL(msr.getNOutputs() == 1);
		CHECK_FATAL(msr.getOutput(0).getDatatype() == Tensor::Short);

		if (histo)
		{
			CHECK_FATAL(histoEqual.process(msr.getOutput(0)) == true);
		...
		else
		{
			// Remove the third dimension and save the result to a file
			out_2d=new ShortTensor();
			out_2d->select( &((const ShortTensor&)msr.getOutput(0)), 2, 0 );
		}
